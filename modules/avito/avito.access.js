...
data.append('client_id', 'rNnFe3TEsIkmH-H2kpfg');
data.append('client_secret', 'pdzp0TqUAf7IncsSJYharPPpfrt7OdCAEjwzsRqu');
...
const avitoTokenObject = {
   ...
   getNewToken: async function() {
      try {
         ...
         const date = new Date()
         fs.writeFileSync('./modules/avito/tokenAvito.txt', `${newTokenValue}\n${date}`)         
         return 
      } catch (error) {
         console.log(error)
      }
   ...
   readTokenFromFile: async function() {
      try {
         const data = await fs.promises.readFile('./modules/avito/tokenAvito.txt', 'utf8');
...
module.exports = avitoTokenObject